...
Iberian_all <- read_excel(paste(wd,"/Master Project/Data/Input data/iberia_pollen_records_v2.xlsx",sep=""))
...
date_control <- read_excel(paste(wd,"/Master Project/Reconstruction paper/3 Revision/Revision 3/From Yicheng/Iberia_pollen_dates_tomeng.xlsx",sep=""))
...
p1<-ggplot() + geom_polygon(data = region, aes(x=long, y = lat, group = group),fill='gray60',color='gray30') + theme_bw()+
  ...
  labs(x=NULL,y="Latitude",colour ="(a) MTCO (°C)")+
...
p2<-ggplot() + geom_polygon(data = region, aes(x=long, y = lat, group = group),fill='gray60',color='gray30') + theme_bw()+
  ...
  scale_x_continuous(breaks = xat, labels = paste0(xat,'°E')) +
  theme(axis.text=element_text(size=12),legend.position = "right")+
  labs(x=NULL,y="Latitude",colour ="(b) MTWA (°C)")+
  theme(axis.text.x = element_blank())
...
p2<-ggplot()+theme_bw()+geom_point(data=CRU,aes(Tmax,alpha),color="grey",size=1)+
  ...
  labs(y=expression(alpha),x="MTWA (°C)")+
...
p1<-ggplot(data=gradient_env_high[which(!is.na(gradient_env_high$Tmin_anomaly)),],aes(x=fct_reorder(site, lon),y=age,color=Tmin_anomaly))+
  ...
  scale_colour_gradientn(colours = c("darkblue","blue","dodgerblue2","lightskyblue","white","orange","orangered"),
                         limits=c(-28,14),na.value = "transparent")+
...
########################################################################################################
if(!require(sf)){ install.packages("sf");library(sf)}
gradient_env<-read.csv(paste(wd,"/Master Project/Data/Output data/Core/Iberian/gradient_env.csv",sep=""),row.names = 1)
