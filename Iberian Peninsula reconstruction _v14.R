...
`%>%` <- magrittr::`%>%`
#MTCO
sse_tf_Tmin<-fxTWAPLS::sse.sample(modern_taxa=taxa,modern_climate=modern_pollen$Tmin,fossil_taxa=core,trainfun=TWAPLS.w2,predictfun=fxTWAPLS::TWAPLS.predict.w,
                                         nboot=1000,nPLS=5,nsig=4,usefx=TRUE,fx_method = "pspline",bin=0.02)%>%fxTWAPLS::pb()
#GDD0
sse_tf_gdd<-fxTWAPLS::sse.sample(modern_taxa=taxa,modern_climate=modern_pollen$gdd,fossil_taxa=core,trainfun=TWAPLS.w2,predictfun=fxTWAPLS::TWAPLS.predict.w,
                                        nboot=1000,nPLS=5,nsig=2,usefx=TRUE,fx_method = "pspline",bin=20)%>%fxTWAPLS::pb()
#alpha
sse_tf_alpha<-fxTWAPLS::sse.sample(modern_taxa=taxa,modern_climate=modern_pollen$alpha,fossil_taxa=core,trainfun=TWAPLS.w2,predictfun=fxTWAPLS::TWAPLS.predict.w,
                                          nboot=1000,nPLS=5,nsig=3,usefx=TRUE,fx_method = "pspline",bin=0.002)%>%fxTWAPLS::pb()
#MTWA
sse_tf_Tmax<-fxTWAPLS::sse.sample(modern_taxa=taxa,modern_climate=modern_pollen$Tmax,fossil_taxa=core,trainfun=TWAPLS.w2,predictfun=fxTWAPLS::TWAPLS.predict.w,
                                         nboot=1000,nPLS=5,nsig=4,usefx=TRUE,fx_method = "pspline",bin=0.02)%>%fxTWAPLS::pb()
...
p_this3<-ggplot()+theme_bw()+
  ...
  geom_pointrange(data=mean_site_env,aes(age_ka,mean_alpha_anomaly,
                                         ymin=mean_alpha_anomaly-range_alpha_anomaly,
                                         ymax=mean_alpha_anomaly+range_alpha_anomaly))+
  labs(y=expression(alpha* " anomaly"),x=NULL)+
  scale_x_continuous(breaks= seq(0.5,11.5))+
...
p_Kaufman2<-ggplot()+theme_bw()+
  ...
  geom_pointrange(data=mean_site_summer_Kaufman,aes(age_ka,mean_Tmax_anomaly,
                                                    ymin=mean_Tmax_anomaly-range_Tmax_anomaly,
                                                    ymax=mean_Tmax_anomaly+range_Tmax_anomaly))+
  labs(x=NULL)+scale_x_continuous(breaks= seq(0.5,11.5))+
  scale_y_continuous(breaks= seq(min_Tmax,max_Tmax),limits=c(min_Tmax,max_Tmax))+
  theme(axis.text.y = element_blank(),axis.title.y = element_blank())+
  annotate("text", y= max_Tmax, x =0,label="(h)",size=5)


p<-ggarrange(p_this1,p_Mauri1,p_Kaufman1,p_this2,p_Mauri2,p_Kaufman2,p_this3,p_Mauri3,ncol=3)

ggsave(file="Compare composite curves.jpeg",p,width=11,height=12)
...
age<-rev(rep(seq(1:7850),each=12))
...
ggplot(df1_pr,aes(age_100yrbin,prep))+geom_point()+geom_line()+theme_bw()+
  labs(y=bquote("MPI  mean monthly precipitation (kg"~s^-1~m^-2~")"),x="Age (yr)")
...
ggplot(df2_pr,aes(age_100yrbin,prep))+geom_point()+geom_line()+theme_bw()+
  labs(y=bquote("AWI  mean monthly precipitation (kg"~s^-1~m^-2~")"),x="Age (yr)")
...
ggplot(df3_pr,aes(age_100yrbin,prep))+geom_point()+geom_line()+theme_bw()+
  labs(y=bquote("TR5AS  mean monthly precipitation (kg"~s^-1~m^-2~")"),x="Age (yr)")
...
ggplot(df4_pr,aes(age_100yrbin,prep))+geom_point()+geom_line()+theme_bw()+
  labs(y=bquote("TR6AV  mean monthly precipitation (kg"~s^-1~m^-2~")"),x="Age (yr)")
...
p3<-ggplot()+theme_bw()+
  ...
  labs(y=bquote("Mean daily precipitation ("~10^-5~kg~s^-1~m^-2~")"),x="Age (yr)")+
