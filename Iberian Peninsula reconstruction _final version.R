...
source("D:/Master Project/Reconstruction paper/Data and codes/Iberian Peninsula reconstruction functions_final version.R")

wd<-"D:/Master Project/Reconstruction paper/Data and codes/"
...
modern_pollen<- read.csv(paste(wd,"Input data/Modern_Pollen_gdd_alpha_Tmin.csv",sep=""), row.names=1)
...
rand_tf2_pspline<-rbind.data.frame(rand_tf2_Tmin_pspline,rand_tf2_Tmax_pspline,rand_tf2_alpha_pspline)
write.csv(rand_tf2_pspline, "Table 2 rand_tf2_pspline.csv")
...
Iberian_all <- read_excel(paste(wd,"Input data/Iberia_pollen_records_v3_0307.xlsx",sep=""))
Iberian<-Iberian_all[which(Iberian_all$`INTCAL2020_median`<=12000),] #choose Holocene period
...
for(k in 1:nlevels(Iberian1$entity_name)){
  ...
  eachsite<-eachsite[order(eachsite$INTCAL2020_median),]
...
#get average resolution after excluding age with large uncertainty
resolution<-data.frame()
Iberian1<-Iberian
Iberian1$entity_name<-as.factor(Iberian1$entity_name)
for(k in 1:nlevels(Iberian1$entity_name)){
...
#adjust fossil taxa to the format of modern taxa
...
setwd(paste(wd,"Output data",sep=""))
#MTCO
...
core_sig<-cbind.data.frame(Iberian[,c("site_name","latitude","longitude","elevation","INTCAL2020_median")],
                           fossil_tf_Tmin[["fit"]][,4], fossil_tf_Tmax[["fit"]][,4],fossil_tf_alpha[["fit"]][,3])
colnames(core_sig)<-c("site","lat","lon","elv","age","Tmin","Tmax","alpha")
write.csv(core_sig,paste(wd,"Output data/Iberian_core_sig.csv",sep=""))
...
sse_core_sig<-cbind.data.frame(sse_tf_Tmin,sse_tf_Tmax,sse_tf_alpha)
colnames(sse_core_sig)<-c("sse_Tmin","sse_Tmax","sse_alpha")
write.csv(sse_core_sig,paste(wd,"/Output data/Iberian_sse_core_sig.csv",sep=""))
...
date_control <- read_excel(paste(wd,"Input data/Iberia_data_dates_v3.xlsx",sep=""))
...
write_xlsx(sitelist,paste(wd,"Output data/Table 1 sitelist.xlsx",sep=""))
write_xlsx(date_control,paste(wd,"Output data/date_control.xlsx",sep=""))

########################################################################################################
###############################    CCA  Table 3 and Figure S11  ########################################
########################################################################################################
if(!require(vegan)){ install.packages("vegan");library(vegan)}
modern_MTCO<-modern_pollen$Tmin
modern_MTWA<-modern_pollen$Tmax
modern_alpha<-modern_pollen$alpha
modern_ord<-cca(formula=taxa~modern_MTCO+modern_MTWA+modern_alpha)
modern_ord
round(vif.cca(modern_ord),digits=2)
round(modern_ord[["CCA"]][["biplot"]],digits = 3)
anova(modern_ord)
anova(modern_ord, by="term", permutations=999)
anova(modern_ord, by="axis", permutations=999)
plot(modern_ord,display=c("species","bp"))


palaeo_MTCO<-core_sig$Tmin
palaeo_MTWA<-core_sig$Tmax
palaeo_alpha<-core_sig$alpha
palaeo_ord<-cca(formula=core~palaeo_MTCO+palaeo_MTWA+palaeo_alpha,na.action=na.omit)
palaeo_ord
round(vif.cca(palaeo_ord),digits=2)
round(palaeo_ord[["CCA"]][["biplot"]],digits = 3)
anova(palaeo_ord)
anova(palaeo_ord, by="term", permutations=999)
anova(palaeo_ord, by="axis", permutations=999)
plot(palaeo_ord,display=c("species","bp"),xlim=c(-7,7))


########################################################################################################
##########################   Find out taxa contribution Table S2  ######################################
########################################################################################################
...
contri_top10_down<-cbind.data.frame(u_t2_Tmin_keep[(nrow(u_t2_Tmin_keep)-9):nrow(u_t2_Tmin_keep),"taxa"],
                                    u_t2_Tmax_keep[(nrow(u_t2_Tmax_keep)-9):nrow(u_t2_Tmax_keep),"taxa"],
                                    u_t2_alpha_keep[(nrow(u_t2_alpha_keep)-9):nrow(u_t2_alpha_keep),"taxa"])
write.csv(contri_top10_up,paste(wd,"Output data/Table S2 contri_top10_up.csv",sep=""))
write.csv(contri_top10_down,paste(wd,"Output data/Table S2 contri_top10_down.csv",sep=""))
...
write.csv(plotdata,paste(wd,"Output data/plotdata.csv",sep=""))
unique(plotdata$site)
nrow(plotdata)

########################################################################################################
#####################   Relationship between MTWA and alpha Figure 6  ##################################
########################################################################################################
#Figure 6
setwd(paste(wd,"Output data",sep=""))
...
p1<-ggplot(modern_pollen,aes(alpha,Tmax))+geom_point(size=0.8)+theme_bw()+
  labs(x= expression(alpha), y = expression("MTWA ("*degree*C*")"))+ylim(0,35)+
...
ggsave(file="Figure 6 Relationship between MTWA and alpha.jpeg",p,width=12,height=6)
...
p1<-ggplot() + geom_polygon(data = region, aes(x=long, y = lat, group = group),fill='gray60',color='gray30') + theme_bw()+
  ...
  labs(x=NULL,y="Latitude",colour =expression("(a) MTCO ("*degree*C*")"))+
...
p2<-ggplot() + geom_polygon(data = region, aes(x=long, y = lat, group = group),fill='gray60',color='gray30') + theme_bw()+
  ...
  scale_x_continuous(breaks = xat, labels = paste0(xat,'°E')) +
  theme(axis.text=element_text(size=12),legend.position = "right")+
  labs(x=NULL,y="Latitude",colour =expression("(b) MTWA ("*degree*C*")"))+
  theme(axis.text.x = element_blank())
...
ggsave(file="Figure S1 Map of SMPDS climate.jpeg",p,width=10,height=12)

########################################################################################################
##############################  Maps of the climate space Figure 1  ####################################
########################################################################################################
library(raster)
...
CRU_Tmin_gdd<-read.csv(paste(wd,"Input data/CRU background/CRU Europe Sep18.csv",sep=""),row.names = 1)
...
#from Roberto
CRU_MI<-brick(paste(wd,"Input data/CRU background/cru_ts4.04-clim-1961-1990-mi.nc",sep=""))
...
CRU_gdd<-brick(paste(wd,"Input data/CRU background/cru_ts4.04-clim-1961-1990-daily.tmp-gdd0.nc",sep=""))
...
p1<-ggplot()+theme_bw()+geom_point(data=CRU,aes(Tmin,Tmax),color="grey",size=1)+
  ...
  labs(y=expression("MTWA ("*degree*C*")"),x=expression("MTCO ("*degree*C*")"))+
...
p2<-ggplot()+theme_bw()+geom_point(data=CRU,aes(Tmax,alpha),color="grey",size=1)+
  ...
  labs(y=expression(alpha),x=expression("MTWA ("*degree*C*")"))+
  annotate("text", y= max(CRU$alpha), x =min(CRU$Tmax),label="(b)",size=5)
p<-ggarrange(p1,p2,ncol=2)
ggsave(file="Figure 1 Climate space.jpeg",p,width=8,height=4)
...
#Map at 0.5 ka
setwd(paste(wd,"Output data",sep=""))
background1 <- read.csv(paste(wd,"Input data/add on region_1.csv",sep=""), row.names=1)
background2 <- read.csv(paste(wd,"Input data/add on region_2.csv",sep=""))
...
p1<-ggplot()+geom_point(data=Iberian_background,aes(lon,lat,color=Tmin),size=3,shape=15)+theme_bw()+
  ...
  scale_colour_gradientn(colours = c("blue","dodgerblue2","lightskyblue","white","gold","orangered","red"),na.value = "grey")+
  labs(x="Longitude",y="Latitude",colour =expression("MTCO ("*degree*C*")"))+
...
p2<-ggplot()+geom_point(data=Iberian_background,aes(lon,lat,color=Tmax),size=3,shape=15)+theme_bw()+
  ...
  geom_point(data=sitelist,aes(lon,lat,shape=elv_label),size=2)+
  scale_colour_gradientn(colours = c("blue","dodgerblue2","lightskyblue","white","gold","orangered","red"),na.value = "grey")+
  labs(x="Longitude",y="Latitude",colour =expression("MTWA ("*degree*C*")"))+
  scale_x_continuous(breaks = c(-10,-5,0,5),labels=c("10 °W","5 °W","0 °E","5 °E"))+
  scale_y_continuous(breaks = c(36,38,40,42,44),labels=c("36 °N","38 °N","40 °N","42 °N","44 °N"))+ 
  scale_shape_manual(values=c("high"=17,"low"=15),guide = 'none')+
  theme(axis.title.x=element_blank(),
...
p<-ggarrange(p1,p2,p3,ncol=1)
ggsave(file="Figure 2 Map of modern climate at Iberian Peninsula.jpeg",p,width=6,height=10)
...
write.csv(gradient_env,paste(wd,"Output data/gradient_env.csv",sep=""))

########################################################################################################
#################################   "Hovemoller" type plots Figure 3  ##################################
########################################################################################################
setwd(paste(wd,"Output data",sep=""))

#"Hovemoller" type plots showing the anomalies in MTCO, MTWA and alpha relative to 0.5 ka
#anomaly to lon
...
p1<-ggplot(data=gradient_env_high[which(!is.na(gradient_env_high$Tmin_anomaly)),],aes(x=fct_reorder(site, lon),y=age,color=Tmin_anomaly))+
  ...
  labs(color=expression("(a) MTCO ("*degree*C*")"))+labs(y="Age (kyr BP)")
#min=-max
p2<-ggplot(data=gradient_env_high[which(!is.na(gradient_env_high$Tmax_anomaly)),],aes(x=fct_reorder(site, lon),y=age,color=Tmax_anomaly))+
  ...
  theme(axis.text.x = element_blank(),axis.title.x = element_blank())+
  labs(color=expression("(b) MTWA ("*degree*C*")"))+labs(y="Age (kyr BP)")
#min=-max
...
p4<-ggplot(data=gradient_env_low[which(!is.na(gradient_env_low$Tmin_anomaly)),],aes(x=fct_reorder(site, lon),y=age,color=Tmin_anomaly))+
  ...
  scale_colour_gradientn(colours = c("darkblue","dodgerblue2","lightskyblue","white","orange","orangered","red"),
                         limits=c(-18,18),na.value = "transparent")+
  theme(axis.text.x = element_blank(),axis.title.x = element_blank())+
  labs(color=expression("(d) MTCO ("*degree*C*")"))+labs(y="Age (kyr BP)")
#min=-max
p5<-ggplot(data=gradient_env_low[which(!is.na(gradient_env_low$Tmax_anomaly)),],aes(x=fct_reorder(site, lon),y=age,color=Tmax_anomaly))+
  ...
  scale_colour_gradientn(colours = c("darkblue","dodgerblue2","lightskyblue","white","orange","orangered","red"),
                         limits=c(-9,9),na.value = "transparent")+
  theme(axis.text.x = element_blank(),axis.title.x = element_blank())+
  labs(color=expression("(e) MTWA ("*degree*C*")"))+labs(y="Age (kyr BP)")
#min=-max
...
ggsave(file="Figure 3 Anomaly to lon.jpeg",p,width=8,height=9)
...
write.csv(mean_site_env,paste(wd,"Output data/mean_site_env.csv",sep=""))
...
p1<-ggplot()+theme_bw()+
  ...
  labs(y=expression("MTCO anomaly ("*degree*C*")"))+scale_x_continuous(breaks= seq(0.5,11.5))+
  theme(axis.title.x = element_blank())+
...
p2<-ggplot()+theme_bw()+
  ...
  labs(y=expression("MTWA anomaly ("*degree*C*")"))+scale_x_continuous(breaks= seq(0.5,11.5))+
  theme(axis.title.x = element_blank())+
...
p4<-ggplot()+theme_bw()+
  ...
  scale_x_continuous(breaks= seq(0.5,11.5))+
  annotate("text", y= max(mean_site_env$mean_insol_summer_anomaly,na.rm=T), x =-0.5,label="(e)",size=5)
...
p5<-ggplot()+theme_bw()+
  ...
  scale_x_continuous(breaks= seq(0.5,11.5))+  
  theme(axis.title.x = element_blank())+
  annotate("text", y= max(mean_site_env$mean_insol_winter_anomaly,na.rm=T), x =-0.5,label="(d)",size=5)

p<-ggarrange(p1,p5,p2,p4,p3,ncol=2)
ggsave(p,file="Figure 4 Time series.jpeg",width=10,height=10)
...
write.csv(df_model,paste(wd,"Output data/Table 4 df_model.csv",sep="")) 
...
p<-ggplot(data=plot_gradient,aes(lon,alpha_anomaly))+geom_point()+theme_bw()+
  geom_smooth(method="lm",formula="y~x",col="red")+labs(x="Longitude (°E)",y=expression(alpha~"anomaly to 0.5 ka"))+
...
ggsave(file="Figure 5 alpha to lon.jpeg",p,width=8,height=8)
...
ggsave(file="Figure S4 alpha to elv.jpeg",p,width=8,height=8)
...
p<-ggplot(data=plot_gradient,aes(lon,Tmax_anomaly))+geom_point()+theme_bw()+
  geom_smooth(method="lm",formula="y~x",col="red")+labs(x="Longitude (°E)",y="MTWA (°C) anomaly to 0.5 ka")+
  facet_wrap(vars(agef),nrow=3)+geom_abline(intercept=0,slope=0)+ 
  theme(strip.background = element_blank(),strip.text.x = element_blank())+ 
  geom_text(data = dat_text, size=4,mapping = aes(x = -2.5, y = 7, label = label))
ggsave(file="Figure S5 Tmax to lon.jpeg",p,width=8,height=8)
...
p<-ggplot(data=plot_gradient,aes(elv_km,Tmax_anomaly))+geom_point()+theme_bw()+
  geom_smooth(method="lm",formula="y~x",col="red")+labs(x="Elevation (km)",y="MTWA (°C) anomaly to 0.5 ka")+
  facet_wrap(vars(agef),nrow=3)+geom_abline(intercept=0,slope=0)+ 
  theme(strip.background = element_blank(),strip.text.x = element_blank())+ 
  geom_text(data = dat_text, size=4,mapping = aes(x = 1.5, y = 7, label = label))
ggsave(file="Figure S7 Tmax to elv.jpeg",p,width=8,height=8)
...
p<-ggplot(data=plot_gradient,aes(lon,Tmin_anomaly))+geom_point()+theme_bw()+
  geom_smooth(method="lm",formula="y~x",col="red")+labs(x="Longitude (°E)",y="MTCO (°C) anomaly to 0.5 ka")+
  facet_wrap(vars(agef),nrow=3)+geom_abline(intercept=0,slope=0)+ 
  theme(strip.background = element_blank(),strip.text.x = element_blank())+ 
  geom_text(data = dat_text, size=4,mapping = aes(x = -2.5, y =-20, label = label))
ggsave(file="Figure S2 Tmin to lon.jpeg",p,width=8,height=8)
...
p<-ggplot(data=plot_gradient,aes(elv_km,Tmin_anomaly))+geom_point()+theme_bw()+
  geom_smooth(method="lm",formula="y~x",col="red")+labs(x="Elevation (km)",y="MTCO (°C) anomaly to 0.5 ka")+
  facet_wrap(vars(agef),nrow=3)+geom_abline(intercept=0,slope=0)+ 
  theme(strip.background = element_blank(),strip.text.x = element_blank())+ 
...
ggsave(file="Figure S3 alpha west-east gradient to elv.jpeg",p1,width=8,height=6)

p2<-ggplot(data=plot_gradient[which(plot_gradient$age!=500),],aes(lon,Tmax_anomaly))+geom_point(size=1)+theme_bw()+
  geom_smooth(method="lm",formula="y~x",col="red")+labs(x="Longitude (°E)",y="MTWA (°C) anomaly to 0.5 ka")+
  facet_grid(elv_label~agef)+geom_abline(intercept=0,slope=0)
ggsave(file="Figure S6 Tmax west-east gradient to elv.jpeg",p2,width=8,height=6)
...
for(i in c(100,c(1:12)*1000)){
  mauri_age<-brick(paste(wd,"Input data/Results to compare/Mauri paper/epoch2_tanom_djf_",i,".tif",sep=""))
...
d_summer<-data.frame()
for(i in c(100,c(1:12)*1000)){
  mauri_age<-brick(paste(wd,"Input data/Results to compare/Mauri paper/epoch2_tanom_jja_",i,".tif",sep=""))
  mauri_age <- rasterToPoints(mauri_age, spatial=TRUE)# Convert raster to SpatialPointsDataFrame
  mauri_age <- data.frame(mauri_age@data, lon=coordinates(mauri_age)[,1],lat=coordinates(mauri_age)[,2])    # Assign coordinates to @data slot                     
  mauri_age_Iberian<-mauri_age[which(mauri_age$lon>=-10&mauri_age$lon<=5&mauri_age$lat>=36&mauri_age$lat<=44),]
  mauri_age_Iberian_mean<-colMeans(mauri_age_Iberian)[1]
...
d_alpha<-data.frame()
for(i in c(100,c(1:12)*1000)){
  mauri_age<-brick(paste(wd,"Input data/Results to compare/Mauri paper/epoch2_anom_alpha_",i,".tif",sep=""))
  mauri_age <- rasterToPoints(mauri_age, spatial=TRUE)# Convert raster to SpatialPointsDataFrame
  mauri_age <- data.frame(mauri_age@data, lon=coordinates(mauri_age)[,1],lat=coordinates(mauri_age)[,2])    # Assign coordinates to @data slot                     
  mauri_age_Iberian<-mauri_age[which(mauri_age$lon>=-10&mauri_age$lon<=5&mauri_age$lat>=36&mauri_age$lat<=44),]
...
D <- readLipd(paste(wd,"Input data/Results to compare/Kaufman paper/Temp12k_v1_0_0_LiPD",sep=""))
...
p1<-ggplot(D_Iberian_winter[which(D_Iberian_winter$site%in%unique(gradient_winter_Kaufman$site)),],aes(age_kyr,Tmin,color=site))+
  ...
  labs(x=NULL,y="MTCO (°C)")+scale_x_continuous(breaks= seq(0.5,11.5),limits=c(0,12))+
...
p2<-ggplot(D_Iberian_summer[which(D_Iberian_summer$site%in%unique(gradient_summer_Kaufman$site)),],aes(age_kyr,Tmax,color=site))+
  ...
  labs(x="Age (kyr)",y="MTWA (°C)")+scale_x_continuous(breaks= seq(0.5,11.5),limits=c(0,12))+
...
########################### Torroso paper ###############################
Torroso_Tmin_anomaly<-brick(paste(wd,"Input data/Results to compare/Torroso paper/climate_reconstruction.nc",sep=""),varname="tmin_jan_anomaly")
Torroso_Tmax_anomaly<-brick(paste(wd,"Input data/Results to compare/Torroso paper/climate_reconstruction.nc",sep=""),varname="tmax_jul_anomaly")
Torroso_pr_anomaly<-brick(paste(wd,"Input data/Results to compare/Torroso paper/climate_reconstruction.nc",sep=""),varname="annprc_anomaly")
...
p_this1<-ggplot()+theme_bw()+
  ...
  geom_pointrange(data=mean_site_env,aes(age_ka,mean_Tmin_anomaly,
                                         ymin=mean_Tmin_anomaly-range_Tmin_anomaly,
                                         ymax=mean_Tmin_anomaly+range_Tmin_anomaly))+
  labs(y="MTCO anomaly (°C)")+scale_x_continuous(breaks= seq(0.5,11.5,by=2))+
...
p_this2<-ggplot()+theme_bw()+
  ...
  geom_pointrange(data=mean_site_env,aes(age_ka,mean_Tmax_anomaly,
                                         ymin=mean_Tmax_anomaly-range_Tmax_anomaly,
                                         ymax=mean_Tmax_anomaly+range_Tmax_anomaly))+
  labs(y="MTWA anomaly (°C)")+scale_x_continuous(breaks= seq(0.5,11.5,by=2))+
...
ggsave(file="Figure 7 Compare composite curves.jpeg",p,width=11,height=10)
...
#Import data
tas_MPI_nc<-brick(paste(wd,"Input data/Results to compare/PACMEDY/tas_MPI.nc",sep=""))
pr_MPI_nc<-brick(paste(wd,"Input data/Results to compare/PACMEDY/pr_MPI.nc",sep=""))

tas_AWI_nc<-brick(paste(wd,"Input data/Results to compare/PACMEDY/tas_AWI.nc",sep=""),varname="temp2")
pr_AWI_nc<-brick(paste(wd,"Input data/Results to compare/PACMEDY/pr_AWI.nc",sep=""))

tas_TR5AS_nc<-brick(paste(wd,"Input data/Results to compare/PACMEDY/tas_TR5AS.nc",sep=""))
pr_TR5AS_nc<-brick(paste(wd,"Input data/Results to compare/PACMEDY/pr_TR5AS.nc",sep=""))

tas_TR6AV_nc<-brick(paste(wd,"Input data/Results to compare/PACMEDY/tas_TR6AV.nc",sep=""))
pr_TR6AV_nc<-brick(paste(wd,"Input data/Results to compare/PACMEDY/pr_TR6AV.nc",sep=""))
...
write.csv(tas_MPI_Iberian,paste(wd,"Input data/Results to compare/PACMEDY/tas_MPI_Iberian.csv",sep=""))
...
write.csv(pr_MPI_Iberian,paste(wd,"Input data/Results to compare/PACMEDY/pr_MPI_Iberian.csv",sep=""))
...
write.csv(tas_AWI_Iberian,paste(wd,"Input data/Results to compare/PACMEDY/tas_AWI_Iberian.csv",sep=""))
...
write.csv(pr_AWI_Iberian,paste(wd,"Input data/Results to compare/PACMEDY/pr_AWI_Iberian.csv",sep=""))
...
write.csv(tas_TR5AS_Iberian,paste(wd,"Input data/Results to compare/PACMEDY/tas_TR5AS_Iberian.csv",sep=""))
...
write.csv(pr_TR5AS_Iberian,paste(wd,"Input data/Results to compare/PACMEDY/pr_TR5AS_Iberian.csv",sep=""))
...
write.csv(tas_TR6AV_Iberian,paste(wd,"Input data/Results to compare/PACMEDY/tas_TR6AV_Iberian.csv",sep=""))
...
write.csv(pr_TR6AV_Iberian,paste(wd,"Input data/Results to compare/PACMEDY/pr_TR6AV_Iberian.csv",sep=""))
...
##################################### MPI ################################################
tas_MPI_Iberian<-read.csv(paste(wd,"Input data/Results to compare/PACMEDY/tas_MPI_Iberian.csv",sep=""),row.names=1)
pr_MPI_Iberian<-read.csv(paste(wd,"Input data/Results to compare/PACMEDY/pr_MPI_Iberian.csv",sep=""),row.names=1)
...
########################################### AWI ########################################
tas_AWI_Iberian<-read.csv(paste(wd,"Input data/Results to compare/PACMEDY/tas_AWI_Iberian.csv",sep=""),row.names=1)
pr_AWI_Iberian<-read.csv(paste(wd,"Input data/Results to compare/PACMEDY/pr_AWI_Iberian.csv",sep=""),row.names=1)
...
############################################ TR5AS ################################################
tas_TR5AS_Iberian<-read.csv(paste(wd,"Input data/Results to compare/PACMEDY/tas_TR5AS_Iberian.csv",sep=""),row.names=1)
pr_TR5AS_Iberian<-read.csv(paste(wd,"Input data/Results to compare/PACMEDY/pr_TR5AS_Iberian.csv",sep=""),row.names=1)
...
ggplot(df3_winter,aes(age_100yrbin,T))+geom_point()+geom_line()+theme_bw()+
  labs(y="TR5AS  MTCO (°C)",x="Age (yr BP)")
...
ggplot(df3_summer,aes(age_100yrbin,T))+geom_point()+geom_line()+theme_bw()+
  labs(y="TR5AS  MTWA (°C)",x="Age (yr BP)")
...
############################################ TR6AV #########################################
tas_TR6AV_Iberian<-read.csv(paste(wd,"Input data/Results to compare/PACMEDY/tas_TR6AV_Iberian.csv",sep=""),row.names=1)
pr_TR6AV_Iberian<-read.csv(paste(wd,"Input data/Results to compare/PACMEDY/pr_TR6AV_Iberian.csv",sep=""),row.names=1)
...
ggplot(df4_winter,aes(age_100yrbin,T))+geom_point()+geom_line()+theme_bw()+
  labs(y="TR6AV  MTCO (°C)",x="Age (yr BP)")
...
ggplot(df4_summer,aes(age_100yrbin,T))+geom_point()+geom_line()+theme_bw()+
  labs(y="TR6AV  MTWA (°C)",x="Age (yr BP)")
...
p1<-ggplot()+theme_bw()+
  ...
  labs(y="MTCO (°C)",x=NULL)+annotate("text",x=0,y=10,label="(a)")+
  theme(axis.text.x=element_blank())
...
p2<-ggplot()+theme_bw()+
  ...
  labs(y="MTWA (°C)",x=NULL)+annotate("text",x=0,y=23,label="(b)")+
  theme(axis.text.x=element_blank())
...
p<-ggarrange(p1,p2,p3,ncol=1)
ggsave(file="Figure 8 PACMEDY simulations.png",p,width=6,height=10)
...
ggsave(p,file="Figure S8 PACMEDY simulated west-east prep difference.jpeg",width=8,height=7)
...
ggsave(file=paste("Figure S9 Lon",round(lon,digits=2),"Lat",round(lat,digits=2),sitename,"MTWA reconstruction results.jpeg"),p_Tmax,width=8,height=6)
...
if(!require(foreach)){install.packages("foreach");library(foreach)}
#fxTWAPLSv1: tf1 bin
cv_tf1_Tmin_bin <- fxTWAPLS::cv.pr.w(taxa,
                                         modern_pollen$Tmin,
                                         nPLS = 5,
                                         fxTWAPLS::TWAPLS.w,
                                         fxTWAPLS::TWAPLS.predict.w,
...
cv_tf1_Tmax_bin <- fxTWAPLS::cv.pr.w(taxa,
                                         modern_pollen$Tmax,
                                         nPLS = 5,
                                         fxTWAPLS::TWAPLS.w,
                                         fxTWAPLS::TWAPLS.predict.w,
...
cv_tf1_alpha_bin <- fxTWAPLS::cv.pr.w(taxa,
                                          modern_pollen$alpha,
                                          nPLS = 5,
                                          fxTWAPLS::TWAPLS.w,
                                          fxTWAPLS::TWAPLS.predict.w,
...
rand_compare<-rbind.data.frame(rand_tf1_Tmin_bin,rand_tf2_Tmin_pspline,
                               rand_tf1_Tmax_bin,rand_tf2_Tmax_pspline,
                               rand_tf1_alpha_bin,rand_tf2_alpha_pspline)
write.csv(rand_compare, "Table A1 rand compare.csv")
